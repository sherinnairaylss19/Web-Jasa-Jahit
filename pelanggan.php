                ...
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <thead>
                        <tr style="background-color: #f8f9fa; border-bottom: 2px solid #eee;">
                            ...
                            <th style="padding: 12px; text-align: left;">Tgl Tenggat</th>
                    ...
                    <tbody>
                        ...
                        if ($result) {
                            ...
                            while($row = mysqli_fetch_assoc($result)) {
                                ...
                                // ✅ Logika warna Tgl Tenggat
                                $tgl_tenggat = $row['tgl_tenggat'] ?? '';
                                if ($tgl_tenggat) {
                                    $today = date('Y-m-d');
                                    $selisih = (strtotime($tgl_tenggat) - strtotime($today)) / 86400;
                                    if ($selisih < 0) {
                                        $warna_tenggat = 'color:#dc3545; font-weight:bold;'; // Merah - sudah lewat
                                    } elseif ($selisih <= 3) {
                                        $warna_tenggat = 'color:#fd7e14; font-weight:bold;'; // Oranye - mepet
                                    } else {
                                        $warna_tenggat = 'color:#198754;'; // Hijau - aman
                                    }
                                    $tampil_tenggat = date('d M Y', strtotime($tgl_tenggat));
                                } else {
                                    $warna_tenggat = 'color:#999;';
                                    $tampil_tenggat = '-';
                                }
                                ...
                                // ✅ Kolom Tgl Tenggat baru
                                echo "<td style='padding: 12px; border-bottom: 1px solid #eee; $warna_tenggat'>" . $tampil_tenggat . "</td>";

                                echo "<td style='padding: 12px; border-bottom: 1px solid #eee;'>" . htmlspecialchars($row['jenis_pesanan'] ?? '-') . "</td>";
                                echo "<td style='padding: 12px; border-bottom: 1px solid #eee; font-size:13px; color:#444;'>" . nl2br(htmlspecialchars($row['catatan'] ?? '-')) . "</td>";
