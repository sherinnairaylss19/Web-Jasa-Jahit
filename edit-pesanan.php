...
$stmt = mysqli_prepare($koneksi, "SELECT pesanan.*, pelanggan.nama_lengkap, pelanggan.no_hp, pelanggan.alamat_lengkap 
                                   ...
                                   WHERE pesanan.id_pesanan = ? AND pesanan.is_deleted = 0");
...
if (isset($_POST['update'])) {
    ...
    // Ukuran celana
    $l_pinggang = mysqli_real_escape_string($koneksi, trim($_POST['lingkar_pinggang'] ?? ''));
    $l_pinggul  = mysqli_real_escape_string($koneksi, trim($_POST['lingkar_pinggul']  ?? ''));
    $l_paha     = mysqli_real_escape_string($koneksi, trim($_POST['lingkar_paha']     ?? ''));

    // panjang_baju dipakai untuk atasan maupun celana (kolom sama di DB)
...
<head>
    ...
    <style>
        ...
        #manual-jenis-wrap { margin-top: 8px; display: none; }
        #manual-jenis-wrap input {
            width: 100%; padding: 8px; box-sizing: border-box;
            border: 1px solid #d1d5db; border-radius: 6px; font-size: 14px;
        }
        #manual-jenis-wrap .hint { font-size: 12px; color: #6b7280; margin-top: 4px; }
    </style>
</head>
<body>
<div class="container">
                    ...
                    <div class="form-column">
                        ...
                        <div class="input-group">
                            ...
                            <select name="status_produksi" style="width:100%; padding:8px;">
                                <?php
                                $status_list = ['Proses', 'Selesai', 'Diambil'];
                    ...
                    <div class="form-column">
                        ...
                        <div class="input-group">
                            ...
                            <select id="jenis_select" style="width:100%; padding:8px;" onchange="handleJenisChange(this.value)">
                                ...
                                <?php foreach (['Kemeja','Atasan','Celana','Celana Jeans','Gamis','Kebaya','Seragam'] as $j): ?>
...
(function init() {
    ...
    if (select.value === 'lainnya') {
        ...
        tampilkanUkuran(finalInput.value);
    }
})();
</script>
</body>
